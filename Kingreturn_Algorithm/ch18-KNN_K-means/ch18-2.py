...
def knn(record, target, k):
    ...
    distances = []  # 儲存記錄與目標的距離
    record_number = []  # 儲存記錄的烤香腸數量

    for r in record:  # 計算過往記錄與目標的距離
        tmp = 0
...
target = [1, 5, 2, 'value']  # value是需要計算的值
# 過往記錄
record = [
    [0, 3, 3, 100],
    [2, 4, 3, 250],
...
print('需準備%d條烤香腸' % regression(k_nn[0]))
for i in range(k):
